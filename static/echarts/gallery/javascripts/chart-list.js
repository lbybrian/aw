...
$(document).ready(function() {
	var t = $("#explore-container .chart-list-panel"),
		e = $("#left-chart-nav ul");
	for(var a in CHART_TYPES) t.append('<h3 class="chart-type-head" id="chart-type-' + a + '">' + CHART_TYPES[a] + "</h3>").append('<div class="row" id="chart-row-' + a + '"></div>'), e.append($("<li>").append('<a class="left-chart-nav-link" id="left-chart-nav-' + a + '" href="#chart-type-' + a + '"><div class="chart-icon"></div><div class="chart-name">' + CHART_TYPES[a] + "</div></a>"));
	for(var i = 0, l = EXAMPLES.length; l > i; ++i) {
		var p = EXAMPLES[i].title || "未命名图表",
			r = EXAMPLES[i].subtitle || "点击查看详情",
			n = $('<div class="col-lg-3 col-md-4 col-sm-6"></div>'),
			d = $('<div class="chart"></div>');
		$("#chart-row-" + EXAMPLES[i].type).append(n.append(d)), $link = $('<a class="chart-link" href="./editor.html?c=' + EXAMPLES[i].id + '"></a>'), d.append($link), $link.append($('<div class="chart-hover"></div>').append('<h4 class="chart-title">' + p + "</h4>").append('<div class="chart-subtitle">' + r + "</div>")).append('<div class="chart-hover-bg"></div>'), $chartArea = $('<img class="chart-area" src="data/thumb/' + EXAMPLES[i].id + '.png" />'), $link.append($chartArea)
	}
	$(".chart-type-head").waypoint(function(t) {
		console.log(t);
		var e = this.element.id.split("-");
		3 === e.length && ($("#left-chart-nav li").removeClass("active"), $("#left-chart-nav-" + e[2]).parent("li").addClass("active"))
	}, {
		offset: 70
	});
