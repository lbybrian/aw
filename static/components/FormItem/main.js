(function () {
  ...
  webCpu.regComponent("FormItem", config, function (container, data, task) {
    ...
    for (var i = 0; data.items && i < data.items.length; i++) {
      ...
      if (data.items[i].type === "select") {
        temp = $('<label key="' + data.name + '" style="width: 100%;" type="select" class="FormItem_inputItem"></label>');
        webCpu["FormItem"].addSelectItem(temp, data.items[i].options, data.value);

      } else if (data.items[i].type === "textarea") {
        temp = $('<label key="' + data.name + '" style="width: 100%;" type="' + data.items[i].type + '" class="FormItem_inputItem"><textarea value="' + data.items[i].value + '" name="' + data.name + '" type="' + data.items[i].type + '" ></textarea>' + (data.items[i].tips || '') + '</label>');
      } else if (data.items[i].type === "text" || data.items[i].type === "password") {
        temp = $('<label key="' + data.name + '" style="width: 100%;" unit="' + (data.items[i].unit || '') + '" type="' + data.items[i].type + '" class="FormItem_inputItem"><input  name="' + data.name + '" type="' + data.items[i].type + '" value="' + (data.items[i].value || "") + '"  placeholder="' + (data.items[i].value || "") + '" /><span style="vertical-align: middle;">' + (data.items[i].tips || '') + '</span></label>');
      } else {
        temp = $('<label key="' + data.name + '"  style="width: auto;" unit="' + (data.items[i].unit || '') + '" type="' + data.items[i].type + '" class="FormItem_inputItem"><input  name="' + data.name + '" type="' + data.items[i].type + '" value="' + (data.items[i].value || "") + '"  /><span style="vertical-align: middle;">' + (data.items[i].tips || '') + '</span></label>');
      }
      temp.appendTo($(t));
  ...
  webCpu["FormItem"].addSelectItem = function (elem, data, value) {
    var tMap = {}
    for(var i = 0; data && i < data.length; i++) {
      var item = data[i];
      if(item.name) {
        tMap[item.value] = item.name;
      }
      else {
        tMap[item] = item
      }
    ...
    var temp = '<div class="input-group-btn" style="border:none; background: transparent;">\
          <a><input disabled style="cursor: pointer;" type="text" class="form-control" aria-label="..." data="' + value + '" value="' + (tMap[value] || "") + '"></a>\
