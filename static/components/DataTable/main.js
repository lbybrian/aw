webCpu.regComponent("DataTable", {
  ...
  task.option.valueMap = task.option.valueMap || {};
  ...
  if (task.header && task.header.length > 0) {
    data = data || [];
    ...
    for (var i = 0; i < data.length; i++) {
      ...
      for (var j = 0; j < task.header.length; j++) {
        ...
        if (typeof (task.header[j].render) === "function") {
          ...
          if(task.header[j].titleTips === "always") {
            v = str;
          }
          ...
          if (data[i][k] !== undefined) {
            var str = data[i][k];
            if(task.option.valueMap[k] && task.option.valueMap[k][str]) {
              str = task.option.valueMap[k][str];
            }
            v = str;
          }
        }
    ...
    if (i === 0) {
      var tr = document.createElement("tr");
      tr.innerHTML = "<td style='color: #111;' colspan='" + count + "'><p class='DataTable_emptyTip'>当前列表为空</p></td>";
      tbody.appendChild(tr);
    }
