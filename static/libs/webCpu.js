...
WebAdapter.loadCardData = function (url, key, myCallback) {
  ...
  window[key] = function (d) {
    ...
    // var item = _self.jsonpCallback[key].pop();
    // if(item && item.id === id && item.callback) {
    //   item.callback(d);
    // }
...
var WebRequest = function (url, type, options, callback) {
  var type = type || "jsonp";
  if (type === "jsonp" || type === "noEcho") {
    return (new CustomInterface(url, type, options, callback));
  } else {
    return (new AjaxInterface(url, type, options, callback));
  }
}
...
var AjaxInterface = function (url, type, dType, callback) {
  this.url = url;
  ...
  if (this.httpObj) {
    ...
    return (function (query, callback, dsl) {
      ...
      if(typeof(_this.callback) === "function") {
        _this.callback(query, _this.httpObj);
      }
      ...
      if(dsl) {
        _this.httpObj.setRequestHeader("Content-Type", "application/json; charset=UTF-8");
        data = dsl;
      }
      else {
...
ViewControl.prototype.updateView = function (task, tData) {
  ...
  if (task.option && task.footArea && webCpu.CardItem && webCpu.CardItem.Pagination && task.option.page && task.option.page.total > 0) {
    var page = task.option.page;
    if (typeof (task.option.pageFilter) === "function") {
      page = task.option.pageFilter(page);
    }

    task.footArea.style.display = "block";
    ...
    if(task.footArea) {
      task.footArea.style.display = "none";
    }
  }
  ...
  var callback = function (container, data, task) {
    ...
    for (var k in webCpu.interface) {
      ...
      if (adapter.flag) {
        ...
        _self.adapter[k] = new WebRequest(adapter.url, type, adapter.dataType, adapter.callback);
      }
    }
...
WebCpu.prototype.initWebApp = function (elem, app) {
  ...
  if(app.url) {
    ...
    _self.initProject(elem, app.routerOption, app.titleData);
  }
}
